...
const DatasetNav = ({routes, location, hasGeo, id, dataId, onDownload, onOutboundLink}) => {
  const handleTabSelect = (key, e) => {
    e.preventDefault()
    let pathArray = location.pathname.split('/')
    key = key === 'overview' ? '' : key
    if (key === 'api') {
      onOutboundLink('https://dev.socrata.com/foundry/data.sfgov.org/' + dataId)
    } else {
      ...
      <Row className={'chartTabs DatasetNav'}>
        ...
        <div className={'dataset-nav-btns'}>
          <Nav
            ...
            <DownloadLinks hasGeo={hasGeo} id={id} dataId={dataId} onDownload={onDownload} />
